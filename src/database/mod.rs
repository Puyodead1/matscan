...
impl Database {
    pub async fn connect(mongodb_uri: &str) -> anyhow::Result<Self> {
        ...
        while let Some(Ok(doc)) = cursor.next().await {
            if let Some(addr) = get_u32(&doc, "ip") {
    ...
    pub fn mcscanner_database(&self) -> mongodb::Database {
        self.client.database("cope")
    }

    pub fn servers_coll(&self) -> Collection<Document> {
        self.mcscanner_database().collection::<Document>("cachedservers")
    }
...
pub async fn collect_all_servers(
    ...
    while let Some(doc) = cursor.try_next().await? {
        let ip_str = match doc.get_str("ip").ok() {
            Some(ip) => ip,
            None => continue,
        };

        let ip = match ip_str.parse::<Ipv4Addr>().ok() {
            Some(addr) => addr,
            None => continue,
        };
        ...
        servers.push(SocketAddrV4::new(ip, port as u16));
